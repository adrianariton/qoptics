...
md"""Below we plot numerical and analytical solutions for the components of both $|00\rangle$ and $|11\rangle$ ($c_0$ and $c_1$ respectively) of state $|\psi\rangle$ with respect to time. 
...
|:-------------|:------|:------------------|:---------|:--------|
|`$(round(g, digits=3))` kHz      | `$nₚ` | `$(round(gᵖʳᶦᵐᵉ, digits=3))` MHz     | `[0, $(tᶠᶦⁿ)]` µs |`$γₑ` MHz|
""")
...
begin
	...
	deltat = tᶠᶦⁿ/1000 		# sampling only 1000 equidistant points
	t = 0:deltat:tᶠᶦⁿ 		# time range

	# analytical components of ψ
...
Markdown.parse("""
	|g / γₑ            | Comments 				   |
	|:-----------------|:--------------------------|
	|`$(Printf.format(Printf.Format("%.3e"), g * kHz / (γₑ * MHz)))`      | `$(get_regime(g, γₑ))`    |
	""")
...
begin
	...
	if approx2
		plot!(t, real(exp.((2 * gᵖʳᶦᵐᵉ - γₑ/2) *MHz * t / factor)), linewidth=1, ls=:dash, label = "Second Aproximation", c=1)
	end
...
|g             | nₚ    | gᵖʳᶦᵐᵉ            |g₀      |γₑ       |
|:-------------|:------|:------------------|:---------|:--------|
|`$(round(g, digits=3))` kHz      | `$(round(nₚ, digits=3))` | `$(round(gᵖʳᶦᵐᵉ, digits=3))` MHz     | `$(round(g₀, digits=3))` kHz |`$(round(γₑ, digits=3))` MHz|
""")
...
md"""Formula implemented above: $\frac{|c_1|^2}{|c_0|^2 + |c_1|^2}$"""
...
md"""*Example:* To see the infidelity grow to around $0.15$, set the sliders to the following values: 
- `log₁₀(g₀ (Hz))`  $= 5$       $\implies g₀ = 100 kHz$
- `log₁₀(nₚ)`       $= 2.5$     $\implies nₚ = 316$
- `log₁₀(γₑ (MHz))` $= 1$       $\implies γₑ = 1 MHz$
"""

# ╔═╡ 02543f9e-0fb1-4f6d-8290-1a14e6fd4ecd
md"""## Workarounds

While the infidelity due to two-photon-excitations is unavoidable, the first can be eliminated in the two ways:
...
The state generated by our heralding process would not be a pure entangled state. Rather, it would be of the form:
...
*$[!]$ in the case of the **blue-detuned** pump*
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "68bb836c822ccf21e086ec7b11544761804e9f00"
